...
void RedSocialEj2::cliqueMasInfluyente(vector<Actor2> &Q, vector<Actor2> &K) const
{
    ...
    else
    {
        ...
        if (influenciaDeGrupo(Q) + influenciaGruposIndependientes(I, K) <= influenciaMaximaVista2)
        {
            return;
        }
        else
        {
            if (influenciaDeGrupo(Q) + influenciaDeGrupo(K) <= influenciaMaximaVista2)
            {
                return;
            }
            else
...
int RedSocialEj2::influenciaDeGrupo(const vector<Actor2> &grupo)
{ // O(|grupo|)
...
void RedSocialEj2::soloAmigosDeQEnK(vector<Actor2> &Q, vector<Actor2> &K) const
{ // O(k^2)
    vector<Actor2> soloAmigosDeQEnK;
    for (auto &i : K)
    {
        if (sonAmigos(Q.back(), i))
            soloAmigosDeQEnK.push_back(i);
    }
    K = soloAmigosDeQEnK;
}

void RedSocialEj2::filtrarPopulares(vector<Actor2> &Q, vector<Actor2> &K, vector<Actor2> &sinPopulares) const
{ // O(k^2)
    for (auto &v : K)
    { // Recorro todos los vertices en k.
        int amigos = 0;
        for (auto &u : K)
            if (sonAmigos(v, u))
                amigos++;
        if (amigos == K.size() - 1)
            Q.push_back(v); // La variable amigos es igual que el tamanio de K - 1 (sin contar a sigo mismo). Por lo tanto es amigo de todos y lo quiero en Q.
        else
            sinPopulares.push_back(v); // Caso contrario se queda en K.
    }
}

bool RedSocialEj2::sonAmigos(Actor2 v, Actor2 u) const
{ // O(1)
