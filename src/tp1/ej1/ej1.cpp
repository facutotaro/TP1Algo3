...
void RedSocialEj1::cliqueMasInfluyente(vector<Actor1> &Q, vector<Actor1> &K) const
{
    ...
    {
        if (influenciaDeGrupo(Q) + influenciaDeGrupo(K) <= influenciaMaximaVista1)
            return;
        else
        {
            vector<Actor1> Qd = Q;
...
int RedSocialEj1::influenciaDeGrupo(const vector<Actor1> &grupo)
{ // O(|grupo|)
...
void RedSocialEj1::soloAmigosDeQEnK(vector<Actor1> &Q, vector<Actor1> &K) const
{ // O(k^2)
...
void RedSocialEj1::filtrarPopulares(vector<Actor1> &Q, vector<Actor1> &K, vector<Actor1> &sinPopulares) const
{ // O(k^2)
    for (auto &v : K)
...
bool RedSocialEj1::sonAmigos(Actor1 v, Actor1 u) const
{ // O(1)
